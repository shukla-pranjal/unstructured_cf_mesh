...
class OBJECT_OT_RunSolver(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        actual_cmd = "foamRun -solver incompressibleFluid" if props.solver_type == 'simpleFoam' else props.solver_type
        command = f"{source_cmd} && {actual_cmd}"
