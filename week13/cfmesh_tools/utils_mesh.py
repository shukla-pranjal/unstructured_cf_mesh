...
def _sanitize_patch_name(name: str) -> str:
    """Return a valid OpenFOAM patch name (no leading digit, no spaces)."""
...
def generate_fields(base_dir, solver_type, nu, inlet_vel, turb_model="laminar", turb_k=0.1, turb_epsilon=0.1, turb_omega=1.0, turb_nut=0.0, start_time=0.0, end_time=0.5, delta_t=0.001, write_interval=20, boundary_patches=None):
    ...
    "sides" {{ type symmetry; }}'''
        ...
        for p in boundary_patches:
            safe_name = _sanitize_patch_name(p.name)
            name = f'"{safe_name}.*"'
            btype = p.bc_type
            if btype == 'wall':
                u_bcs += f"\n    {name} {{ type noSlip; }}"
...
"""
    ...
    if solver_type == 'icoFoam':
        ...
        write_interval_val = int(write_interval)
        end_time_val = int(end_time)
...
def create_case_structure(base_dir, cell_size=0.1, boundary_layers=3, thickness_ratio=1.2, stl_name="mesh.stl", cpu_cores=1, boundary_patches=None, edge_refinements=None, box_refinements=None, surface_refinements=None, cylinder_refinements=None, improve_quality=True, layer_optimise=True, layer_max_iter=5):
    """
...
"""
    ...
    if box_refinements:
        box_refine_block = "\nobjectRefinements\n{"
        for i, br in enumerate(box_refinements):
            bmin = br["min"]
            bmax = br["max"]
            cx = (bmin[0] + bmax[0]) / 2.0
            cy = (bmin[1] + bmax[1]) / 2.0
            cz = (bmin[2] + bmax[2]) / 2.0
            lx = abs(bmax[0] - bmin[0])
            ly = abs(bmax[1] - bmin[1])
            lz = abs(bmax[2] - bmin[2])
            box_refine_block += f"""
    boxRefinement_{i}
    {{
        type box;
        ...
        lengthY {ly};
        lengthZ {lz};
        cellSize {br["cell_size"]};
    }}"""
        box_refine_block += "\n}"

    if box_refinements or cylinder_refinements:
        if not box_refinements:
            box_refine_block = "\nobjectRefinements\n{"
        else:
            # Remove closing brace from box_refine_block to append cylinders
            box_refine_block = box_refine_block[:-2]
            
        if cylinder_refinements:
            for i, cr in enumerate(cylinder_refinements):
                p1 = cr["p1"]
                p2 = cr["p2"]
                box_refine_block += f"""
    cylinderRefinement_{i}
    {{
        type cone;
        p0 ({p1[0]} {p1[1]} {p1[2]});
        p1 ({p2[0]} {p2[1]} {p2[2]});
        radius0 {cr["radius"]};
        radius1 {cr["radius"]};
        cellSize {cr["cell_size"]};
    }}"""
        box_refine_block += "\n}"

    surface_refine_block = ""
    if surface_refinements:
        surface_refine_block = "\nsurfaceMeshRefinement\n{"
        for sr in surface_refinements:
            surface_refine_block += f"""
    {sr["name"]}
    {{
        surfaceFile "constant/triSurface/{sr["file"]}";
        cellSize {sr["cell_size"]};
        refinementThickness {sr["thickness"]};
    }}"""
        surface_refine_block += "\n}"
