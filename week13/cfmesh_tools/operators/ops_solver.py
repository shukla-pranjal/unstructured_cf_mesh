    ...
    def execute(self, context):
        ...
        if props.cpu_cores > 1:
            ...
            if props.solver_type == 'simpleFoam':
                run_solver = f"{cleanup_cmd} && potentialFoam -writephi && decomposePar -force && mpirun -np {props.cpu_cores} simpleFoam -parallel && (reconstructPar -latestTime || true)"
            else:
                run_solver = f"{cleanup_cmd} && decomposePar -force && mpirun -np {props.cpu_cores} {props.solver_type} -parallel && (reconstructPar -latestTime || true)"
        else:
            if props.solver_type == 'simpleFoam':
