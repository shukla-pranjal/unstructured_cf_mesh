...
class VIEW3D_PT_CFMeshPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if global_state.is_running:
            ...
            if hasattr(props, "progress_percent"):
                box.prop(props, "progress_percent", slider=True, text="Progress")
                
            if hasattr(props, "live_log_tail") and props.live_log_tail:
                log_box = box.box()
                log_box.scale_y = 0.8
                log_box.label(text=f"> {props.live_log_tail[:80]}", icon='CONSOLE')
