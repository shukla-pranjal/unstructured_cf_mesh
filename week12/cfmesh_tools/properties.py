...
def update_courant(self, context):
    """Compute Courant number and safe Δt when inlet velocity or cell size changes."""
    U_x, U_y, U_z = self.inlet_velocity
    U_mag = math.sqrt(U_x**2 + U_y**2 + U_z**2)
    dx = self.base_cell_size
    dt = self.delta_t
    if U_mag > 0 and dx > 0:
        co = (U_mag * dt) / dx
        self.courant_number = co
        # Safe Δt = CFL_target * dx / U_mag  (target Co = 0.5)
        self.suggested_dt = (0.5 * dx) / U_mag
    else:
